...
const TYPE_OPTIONS: { value: ContentType; label: string; desc: string; emoji: string }[] = [
...
export default function UnifiedCreationFlow({
    ...
    try {
      ...
      if (genData.variants && Array.isArray(genData.variants)) {
        ...
        const previewVisual = activeLayout
          ? { template: activeLayout as "split" | "centered" | "card", mood: "dark", overlayOpacity: 0.4 }
          : first.visual;
  ...
  const handleVariantSelect = useCallback(
    ...
    [inferredType, variants.length, activeLayout, onGenerated],
  );
  ...
  const goBack = () => {
    ...
    else if (step === "variants") { setStep("brief"); setVariants([]); setGenerating(false); }
  ...
  const toneInfo = activeTone ? TONE_OPTIONS.find((o) => o.value === activeTone) : null;
  const profileTone = !activeTone && contentTone ? TONE_OPTIONS.find((o) => o.value === contentTone) : null;
          ...
          return (
            <ProgressStep key={s}>
              ...
              <ProgressLabel $active={isActive} $done={isDone}>
                {s === "type" ? "Type" : s === "brief" ? "Brief" : s === "variants" ? "Pick" : "Review"}
              </ProgressLabel>
      ...
      <StepBody>
          ...
          <TypeGrid>
            {TYPE_OPTIONS.map((opt) => (
              <TypeCard
                ...
                onClick={() => { onTypeChange(opt.value); setStep("brief"); }}
              >
        ...
        {step === "brief" && (
          <div>
              ...
              <ControlGroup>
                ...
                <PillGroup>
                  {(["fi", "en", "sv"] as const).map((lang) => (
                    <Pill key={lang} $active={language === lang} onClick={() => setLanguage(lang)} disabled={isBusy}>
                      {lang.toUpperCase()}
                    </Pill>
                  ))}
                </PillGroup>
              </ControlGroup>
              <LangHint $visible={language !== "fi"}>
                Generated content will be in {language === "en" ? "English" : "Swedish"}
              </LangHint>
            </ControlsRow>
                ...
                return (
                  ...
                  >
                    <ToneCardTop>
                      ...
                      {isDefault && !isActive && <DefaultBadge>default</DefaultBadge>}
                    </ToneCardTop>
            ...
            {!activeTone && !contentTone && (
              <ToneHint>
                Pick a voice to shape how your cards look and sound. You can skip this — the AI will infer tone from your brief.
              </ToneHint>
            )}
            ...
            <LayoutRow>
              {LAYOUT_HINTS.map((layout) => (
                <LayoutCard
                  ...
                  onClick={() => setActiveLayout(activeLayout === layout.template ? null : layout.template)}
                >
            ...
            <ControlsRow>
              <FormatNote>
                Social cards generated in all 3 formats: Instagram (1:1), Facebook (1.91:1), Cover (3:1)
              </FormatNote>

              <GenerateButton onClick={handleGenerate} disabled={isBusy || !text.trim()}>
                {isBusy ? (
        ...
        {step === "variants" && variants.length > 0 && (
          <div>
            <BriefRecap>
              ...
              {toneInfo && <span style={{ color: "#a78bfa", marginLeft: 8 }}>{toneInfo.emoji} {toneInfo.label}</span>}
            </BriefRecap>
        ...
        {step === "review" && (
          <ReviewSection>
              ...
              <>
                <FieldRow>
                  <FieldGroup style={{ flex: 1 }}>
                    ...
                    <SelectField value={formState.promotionType} onChange={(e) => onFieldChange("promotionType", e.target.value)}>
                      {PROMOTION_TYPES.map((pt) => <option key={pt.value} value={pt.value}>{pt.label}</option>)}
                    </SelectField>
                  </FieldGroup>
                  <FieldGroup style={{ flex: 1 }}>
                    <FieldLabel>Discount (%)</FieldLabel>
                    <FieldInput type="number" value={formState.discountValue ?? ""} onChange={(e) => onFieldChange("discountValue", e.target.value ? Number(e.target.value) : null)} placeholder="e.g. 20" />
                  </FieldGroup>
                </FieldRow>
                <FieldRow>
                  <FieldGroup style={{ flex: 1 }}>
                    <FieldLabel>Start date</FieldLabel>
                    <FieldInput type="date" value={formState.startDate?.slice(0, 10) || ""} onChange={(e) => onFieldChange("startDate", e.target.value)} />
                  </FieldGroup>
                  <FieldGroup style={{ flex: 1 }}>
                    <FieldLabel>End date</FieldLabel>
                    <FieldInput type="date" value={formState.endDate?.slice(0, 10) || ""} onChange={(e) => onFieldChange("endDate", e.target.value)} />
                  </FieldGroup>
                </FieldRow>
                <FieldGroup>
                  <FieldLabel>Conditions / fine print</FieldLabel>
                  <FieldInput value={formState.conditions} onChange={(e) => onFieldChange("conditions", e.target.value)} placeholder="e.g. Valid on Fridays 16:00–19:00" />
                </FieldGroup>
              </>
            )}
              ...
              <>
                <FieldRow>
                  <FieldGroup style={{ flex: 1 }}>
                    ...
                    <FieldInput type="datetime-local" value={formState.startTime?.slice(0, 16) || ""} onChange={(e) => onFieldChange("startTime", e.target.value)} />
                  </FieldGroup>
                  <FieldGroup style={{ flex: 1 }}>
                    <FieldLabel>End time</FieldLabel>
                    <FieldInput type="datetime-local" value={formState.endTime?.slice(0, 16) || ""} onChange={(e) => onFieldChange("endTime", e.target.value)} />
                  </FieldGroup>
                </FieldRow>
                <FieldGroup>
                  <FieldLabel>Max attendees</FieldLabel>
                  <FieldInput type="number" value={formState.maxAttendees ?? ""} onChange={(e) => onFieldChange("maxAttendees", e.target.value ? Number(e.target.value) : null)} placeholder="Leave empty for unlimited" />
                </FieldGroup>
              </>
            )}
            ...
            {contentType === "campaign" && (
              <>
                <FieldGroup>
                  ...
                  <SelectField value={formState.campaignType} onChange={(e) => onFieldChange("campaignType", e.target.value)}>
                ...
                <FieldGroup>
                  ...
                  <FieldInput type="number" value={formState.campaignBudget} onChange={(e) => onFieldChange("campaignBudget", e.target.value ? Number(e.target.value) : 0)} placeholder="e.g. 50" />
                </FieldGroup>
                <FieldRow>
                  <FieldGroup style={{ flex: 1 }}>
                    <FieldLabel>Start date</FieldLabel>
                    <FieldInput type="date" value={formState.campaignStartDate?.slice(0, 10) || ""} onChange={(e) => onFieldChange("campaignStartDate", e.target.value)} />
                  </FieldGroup>
                  <FieldGroup style={{ flex: 1 }}>
                    <FieldLabel>End date</FieldLabel>
                    <FieldInput type="date" value={formState.campaignEndDate?.slice(0, 10) || ""} onChange={(e) => onFieldChange("campaignEndDate", e.target.value)} />
                  </FieldGroup>
                </FieldRow>
              </>
            )}
            ...
            {contentType === "pass" && (
              <FieldRow>
                <FieldGroup style={{ flex: 1 }}>
                  ...
                  <FieldInput value={formState.priceEuros} onChange={(e) => onFieldChange("priceEuros", e.target.value)} placeholder="e.g. 9.90" />
                </FieldGroup>
                <FieldGroup style={{ flex: 1 }}>
                  <FieldLabel>Pass type</FieldLabel>
                  <SelectField value={formState.passType} onChange={(e) => onFieldChange("passType", e.target.value)}>
            ...
            {toneInfo && (
              <ToneTag>{toneInfo.emoji} Generated with {toneInfo.label.toLowerCase()} voice</ToneTag>
            )}

            <SubmitRow>
              <BackLink onClick={goBack} style={{ marginBottom: 0 }}>← Back to options</BackLink>
              <SubmitButton onClick={onSubmit} disabled={submitting || !formState.title.trim()}>
                {submitting ? "Publishing..." : contentType === "campaign" ? "Launch campaign" : "Publish"}
              </SubmitButton>
            </SubmitRow>
          </ReviewSection>
        )}
...
const ProgressDot = styled.div<{ $active: boolean; $done: boolean }>`
  ...
  background: ${({ $active, $done }) => $active ? "#7c3aed" : $done ? "#10b981" : "#2d2d4a"};
...
const ProgressLabel = styled.span<{ $active: boolean; $done: boolean }>`
  ...
  color: ${({ $active, $done }) => $active ? "#a78bfa" : $done ? "#6ee7b7" : "#4b5563"};
...
const ProgressLine = styled.div<{ $done: boolean }>`
  ...
  background: ${({ $done }) => $done ? "#10b981" : "#2d2d4a"}; transition: background 0.3s;
`;

// ---- Shared ----

const StepBody = styled.div` padding: 20px; `;

const StepTitle = styled.h2`
  font-size: 16px; font-weight: 700; color: #f9fafb; margin: 0 0 16px;
  display: flex; align-items: center; gap: 6px;
`;

const StepNum = styled.span` color: #7c3aed; font-size: 14px; `;
...
const SubLabel = styled.div`
  ...
  letter-spacing: 0.05em; margin-bottom: 6px; margin-top: 12px;
`;

const Divider = styled.div` height: 1px; background: #2d2d4a; margin: 16px 0; `;

const BackLink = styled.button`
  display: block; margin-top: 12px; background: none; border: none;
  color: #6b7280; font-size: 12px; cursor: pointer; font-weight: 500; padding: 0;
  &:hover { color: #a78bfa; }
`;

const ErrorBox = styled.div`
  margin-top: 10px; font-size: 12px; color: #ef4444; padding: 8px 12px;
  background: rgba(239, 68, 68, 0.08); border-radius: 8px; border: 1px solid rgba(239, 68, 68, 0.2);
`;

// ---- Step 1: Type ----

const TypeGrid = styled.div` display: grid; grid-template-columns: repeat(2, 1fr); gap: 10px; `;

const TypeCard = styled.button<{ $selected: boolean }>`
  padding: 16px; border: 1px solid ${({ $selected }) => $selected ? "#7c3aed" : "#2d2d4a"};
  border-radius: 10px; background: ${({ $selected }) => $selected ? "rgba(124, 58, 237, 0.1)" : "#0d0d1a"};
  cursor: pointer; text-align: left; transition: all 0.15s; display: flex; flex-direction: column; gap: 6px;
...
const ToneRow = styled.div` display: flex; gap: 6px; flex-wrap: wrap; `;

const ToneCard = styled.button<{ $active: boolean; $default: boolean }>`
  flex: 1 1 100px; min-width: 95px; max-width: 140px; padding: 8px 10px;
  border: 1px solid ${({ $active, $default }) => $active ? "#7c3aed" : $default ? "#4c1d95" : "#2d2d4a"};
  border-radius: 8px;
  background: ${({ $active }) => $active ? "rgba(124, 58, 237, 0.12)" : "#0d0d1a"};
  cursor: pointer; text-align: left; display: flex; flex-direction: column; gap: 4px; transition: all 0.15s;
  ${({ $active }) => $active && "box-shadow: 0 0 0 1px rgba(124, 58, 237, 0.3);"}
  &:hover { border-color: #7c3aed; }
`;

const ToneCardTop = styled.span` display: flex; align-items: center; gap: 4px; `;
const ToneEmoji = styled.span` font-size: 14px; line-height: 1; `;
const ToneLabel = styled.span` font-size: 10px; font-weight: 700; color: #d1d5db; line-height: 1.2; `;
const DefaultBadge = styled.span`
  font-size: 8px; color: #7c3aed; font-weight: 600; background: rgba(124, 58, 237, 0.15);
  padding: 1px 4px; border-radius: 3px; margin-left: auto;
`;

const ToneSample = styled.span<{ $active: boolean }>`
  font-size: 9px; color: ${({ $active }) => $active ? "#a78bfa" : "#6b7280"};
  font-style: italic; line-height: 1.3;
  display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden;
`;

const TonePreview = styled.div`
  margin-top: 10px; padding: 8px 12px; background: rgba(124, 58, 237, 0.08);
...
const ToneHint = styled.div`
  margin-top: 8px; font-size: 11px; color: #4b5563; line-height: 1.5; font-style: italic;
`;

// ---- Templates ----

const TemplateGrid = styled.div` display: grid; grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 6px; `;

const TemplateCard = styled.button<{ $active: boolean }>`
  padding: 10px 12px; border: 1px solid ${({ $active }) => $active ? "#7c3aed" : "#2d2d4a"};
  border-radius: 8px; background: ${({ $active }) => $active ? "rgba(124, 58, 237, 0.12)" : "#0d0d1a"};
  cursor: pointer; text-align: left; display: flex; flex-direction: column; gap: 4px; transition: all 0.15s;
  ${({ $active }) => $active && "box-shadow: 0 0 0 1px rgba(124, 58, 237, 0.3);"}
  &:hover { border-color: #7c3aed; }
`;

const TemplateName = styled.span` font-size: 12px; font-weight: 700; color: #d1d5db; line-height: 1.2; `;
...
const Textarea = styled.textarea`
  width: 100%; padding: 14px 16px; border: 1px solid #2d2d4a; border-radius: 10px;
  font-size: 14px; min-height: 90px; resize: vertical; box-sizing: border-box;
  font-family: inherit; line-height: 1.55; background: #0d0d1a; color: #e5e7eb; transition: border-color 0.2s;
...
const LayoutRow = styled.div` display: flex; gap: 8px; `;

const LayoutCard = styled.button<{ $active: boolean }>`
  flex: 1; padding: 10px; border: 1px solid ${({ $active }) => $active ? "#7c3aed" : "#2d2d4a"};
  border-radius: 8px; background: ${({ $active }) => $active ? "rgba(124, 58, 237, 0.1)" : "#0d0d1a"};
...
const LayoutThumb = styled.div<{ $template: string }>`
  width: 60px; height: ${({ $template }) => $template === "card" ? "60px" : "34px"}; border-radius: 4px;
  background: ${({ $template }) =>
    $template === "split" ? "linear-gradient(90deg, #3b82f6 40%, #1e293b 40%)"
    : $template === "centered" ? "linear-gradient(135deg, #8b5cf6, #1e293b)"
    : "linear-gradient(180deg, #ef4444 45%, #1e293b 45%)"};
`;

const LayoutName = styled.span` font-size: 11px; font-weight: 700; color: #d1d5db; `;
const LayoutDesc = styled.span` font-size: 9px; color: #6b7280; line-height: 1.3; `;

// ---- Controls ----

const ControlsRow = styled.div` display: flex; align-items: flex-end; gap: 16px; margin-top: 14px; `;
const ControlGroup = styled.div` display: flex; flex-direction: column; gap: 4px; `;
const ControlLabel = styled.span` font-size: 10px; font-weight: 600; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em; `;
const FormatNote = styled.span`
  font-size: 10px; color: #6b7280; font-style: italic; flex: 1; text-align: center;
  padding-bottom: 4px;
`;

const LangHint = styled.span<{ $visible: boolean }>`
  font-size: 10px; color: #a78bfa; font-weight: 500; opacity: ${({ $visible }) => ($visible ? 1 : 0)};
  transition: opacity 0.2s;
`;

const PillGroup = styled.div`
  display: flex; gap: 2px; background: #0d0d1a; border-radius: 8px; padding: 2px; border: 1px solid #2d2d4a;
`;

const Pill = styled.button<{ $active: boolean }>`
  padding: 5px 12px; border: none; border-radius: 6px; font-size: 11px; font-weight: 600; cursor: pointer;
  background: ${({ $active }) => $active ? "#7c3aed" : "transparent"};
  color: ${({ $active }) => $active ? "white" : "#6b7280"}; transition: all 0.15s;
  &:hover { color: ${({ $active }) => $active ? "white" : "#d1d5db"}; }
`;

const GenerateButton = styled.button`
  padding: 10px 22px; background: #7c3aed; color: white; border: none; border-radius: 10px;
  font-size: 14px; font-weight: 600; cursor: pointer; transition: all 0.2s; margin-left: auto; white-space: nowrap;
  &:hover:not(:disabled) { background: #6d28d9; }
  &:disabled { opacity: 0.5; cursor: not-allowed; }
`;

const Spinner = styled.span`
  width: 14px; height: 14px; border: 2px solid rgba(255,255,255,0.3); border-top: 2px solid white;
  border-radius: 50%; display: inline-block; animation: spin 0.7s linear infinite;
  @keyframes spin { to { transform: rotate(360deg); } }
`;

const HintRow = styled.div` margin-top: 10px; text-align: center; `;
const HintKey = styled.span`
  font-size: 11px; color: #4b5563; background: #1a1a2e; padding: 1px 6px;
  border-radius: 3px; border: 1px solid #2d2d4a;
`;
const HintText = styled.span` font-size: 11px; color: #4b5563; `;

// ---- Variants ----

const BriefRecap = styled.div`
  font-size: 12px; color: #6b7280; padding: 8px 12px; background: #0d0d1a;
  border-radius: 8px; margin-bottom: 12px; border: 1px solid #2d2d4a; line-height: 1.4;
`;

const BriefLabel = styled.span` font-weight: 600; color: #9ca3af; `;

// ---- Step 4: Review ----

const ReviewSection = styled.div` display: flex; flex-direction: column; gap: 12px; `;

const FieldGroup = styled.div` display: flex; flex-direction: column; gap: 4px; `;
const FieldLabel = styled.label`
  font-size: 11px; font-weight: 600; color: #9ca3af; text-transform: uppercase; letter-spacing: 0.04em;
`;
...
const FieldRow = styled.div` display: flex; gap: 10px; `;

const ToneTag = styled.div`
  font-size: 11px; color: #a78bfa; padding: 6px 10px;
  background: rgba(124, 58, 237, 0.1); border-radius: 6px; font-weight: 500;
`;

const SubmitRow = styled.div`
  display: flex; align-items: center; justify-content: space-between;
  margin-top: 8px; padding-top: 12px; border-top: 1px solid #2d2d4a;
`;

const SubmitButton = styled.button`
  padding: 10px 28px; background: #10b981; color: white; border: none; border-radius: 10px;
