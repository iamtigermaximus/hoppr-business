// src/app/bar/[id]/passes/page.tsx
import { redirect } from "next/navigation";
import { cookies } from "next/headers";
import { authService } from "@/services/auth-service";
import PassManager from "@/components/bar/passes/PassManager";
...
export default async function BarPassesPage({ params }: PageProps) {
  const { id } = await params;
  const { userRole } = await getBarData(id);

  return <PassManager barId={id} userRole={userRole} />;
}
