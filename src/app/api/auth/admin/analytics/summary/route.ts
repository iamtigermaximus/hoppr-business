...
export async function GET(request: NextRequest) {
  try {
    ...
    // Bar type gaps - FIXED: Explicitly cast to BarTypeGap[]
    const allBarTypes = [
      "PUB",
      "CLUB",
      "LOUNGE",
    ...
    const topBarsByCompletion: TopBarByCompletion[] = barsWithCompletionScores
      ...
      .map((bar) => ({
        ...
        city: bar.city,
        district: bar.district,
        completionScore: bar.completionScore,
        profileViews: bar.profileViews,
      }));

    // Bars needing attention - FIXED: Explicitly cast to NeedingAttentionIssue[]
    ...
    try {
      ...
      totalRevenue = vipPasses.reduce(
        (sum, p) => sum + (p.soldCount * p.priceCents) / 100,
        0,
      );
    } catch (error) {
      console.log("Error fetching VIP pass data:", error);
    }
    ...
    try {
      ...
      previousRevenue = prevVipPasses.reduce(
        (sum, p) => sum + (p.soldCount * p.priceCents) / 100,
        0,
      );
    } catch (error) {
